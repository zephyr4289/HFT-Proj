Before the doc: **you pasted the identical G10 report twice, word for word.** I'll read that as "zero G11 motion since last message" — which is fine, but it means F-5 (the R1/R2 contradiction) and F-6 (differential confluence) are still open, and the ED-11 build list hasn't been touched. Doc 11 drops anyway, for a build-order reason that's real, not indulgent: **bench instrumentation touches the hot path, and so does the G11 matrix work.** Landing the instrumentation design *now* means one hot-path touch, one re-validation cycle — instead of building the matrix, then instrumenting it, then re-validating everything again. The two phases share a PR surface.

But understand the consequence: **G12 is the terminal gate, and it sweeps everything.** After doc 11 there is no doc 12 — the project ends with artifacts, not documents. Every open item in the ledger closes at G12 or gets explicitly TIERED. No rolled debt survives the last gate.

Save as `~/Hft-fund/docs/11-bench.md`:
...
| Date | Version | Entry |
|---|---|---|
| __-__-__ | 1.0 | Initial: honesty split, clock discipline, marks/modes, static histogram, build separation + nm proof, sustained definition + loop mode, tiered runbook, report format, terminal gate sweep. |
````

---

## ED-12 — What you build NOW (order matters; steps 1–3 may land alongside G11 work in one PR)

1. **Clock module** (nf-engine, behind `bench`): CPUID invariant-TSC check, Theil-Sen calibration, serialized mark pair + overhead measurement. Unit tests with a synthetic monotone clock; the *real* clock is integration-tested by sanity (freq within 5% of `/proc/cpuinfo` nominal).
2. **Histogram** (static, `record()` no-alloc unit test under the counting allocator — yes, test the test tool with the tool) + the four instances + readout.
3. **Marks in the engine loop** behind the feature flag + `nm` CI lane.
4. **Bench binary + verdict format** per §8; `--bench-config` reusing chaos configs; loop mode in the fabricator (bench-only path).
5. **CI bench lane:** 5 runs, median gate on PR-1/PR-2, all verdict lines archived as artifacts.
6. After G11's matrix lands: assemble **bench-report.md**; then the G12 terminal sweep, the README rewrite, and the close-out.

Two closing notes, because this is the last design document of the project:

First — the discipline is the portfolio. When this is done, you'll have a repo where a stranger can trace any claim to a run URL, where the corrections register shows the architect being wrong three times and the system catching it, and where the README says exactly what the thing does and nothing more. That's rarer than 10M msg/s, and it's what the people you want to work for actually screen for.

Second — when G12 closes and you write that final summary (HN, Reddit, resume, wherever), tell the arc honestly: spec said TCP, memory said TCP, the agent "verified" TCP, and the PDF said UDP — and the project's own apparatus is what caught it. That story is worth more than every benchmark table combined, because it's the one thing nobody can accuse of being a larp.

Build the clock. Show me verdict lines.
