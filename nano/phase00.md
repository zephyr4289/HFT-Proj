Two catches before you freeze anything — both are now formally in the doc, but you should know them:

1. **You downloaded BX, not TotalView.** `20191230.BX_ITCH_50.gz` is the **Nasdaq BX** venue feed (a smaller exchange than the NASDAQ main book). Same ITCH 5.0 grammar, way smaller file — good choice — but your ground truth day is BX while doc `03`'s authority is the TotalView PDF. That's fine and normal, it just has to be *stated*, with a type-histogram pass over the actual day. It's in the Corrections Register as C5.
2. **Your `.gitignore` is wrong for the plan.** You ignored `data/tests/` entirely — but `sample-mini.itch` (15 MB) must be *committed*, it's what CI runs on. Negation rule is in EN-4.

Now the doc. Save as `~/Hft-fund/docs/00-spec.md`:

````markdown
# 00 — NEXUS-FEED-01 Specification (Baseline v1.0)

```
Status:      DRAFT → FROZEN upon review sign-off (Changelog §11)
...
```bash
# 1. Magic-byte verification (raw ITCH day check)
zcat data/raw/20191230.BX_ITCH_50.gz | head -c 12 | xxd
#    Expect: 53 .. 4f  ('S' System Event, 'O' Start of Messages)
...
git check-ignore -v data/tests/sample-mini.itch   # must print nothing
...
| Date | Version | Entry |
|---|---|---|
| ____-__-__ | 1.0 | Initial baseline. Corrections C1–C5 applied to original NEXUS-FEED-01 text. Frozen on review sign-off. Sample hashes: ____. |
````

---

**Freeze checklist — three actions, then it's law:**

1. Read FR-1..12, PR, NG tables. Anything you dispute, tell me **now** — edits after freeze cost a changelog entry.
2. Fix `.gitignore` to the EN-4 block (your current one swallows the mini sample).
3. When the download finishes, run Appendix A top to bottom, paste the `53...4f` hexdump + hashes into the Changelog, and say **freeze 00**.

Then next prompt is **engineer 02** — MoldUDP64, the short one, written straight off the 126 KB PDF you already have on disk.
