Ledger first, one line: still zero G4 evidence attached — and here's the structural fact that makes this the **last doc I can drop on design alone**: doc 05's exit gate is the first *end-to-end* run. A golden hash either matches your chaos replay's output or it doesn't. From this phase forward, no agent on Earth can narrate a gate into passing. The debt either gets paid at G5 or the project visibly stalls. That's not a threat — that's the architecture finally able to enforce itself.

One more thing you need to know before reading: **writing this doc caught a real bug in my own architecture claim.** Doc 01 said "no generation tags, no ABA" — that's still true, but only because of a law I hadn't discovered yet. The naive version of my own window design has a silent-data-corruption bug in it, and the proof obligation is what flushed it out. It's §4 of the doc, called the Zombie Slot Hazard, and it's the single most important thing in this phase. If your agent implements the window without the clear-on-advance law, your engine will *pass most tests* and corrupt bytes in production. Exactly the failure class this project exists to make impossible.

Save as `~/Hft-fund/docs/05-sequencer.md`:
...
| Date | Version | Entry |
|---|---|---|
| __-__-__ | 1.0 | Initial: normative ingest, W1 + zombie hazard + clear-on-advance law, event grammar, gen law, session lifecycle, intent mechanism, U1..U14, E2E-1. |
````

---

## ED-05 — What you build NOW (gate G5)

**1. `nf-arbitrator/src/lib.rs`** — full sequencer per §2/§5. Keep
`#![forbid(unsafe_code)]`. Struct fields EXACTLY as §2 (the layout is the
design). `ingest` follows
Here's the tail end again, verbatim — pick up where your copy stopped.
...
**1. `nf-arbitrator/src/lib.rs`** — full sequencer per §2/§5. Keep
`#![forbid(unsafe_code)]`. Struct fields EXACTLY as §2 (the layout is the
design). `ingest` follows §5 pseudocode line-by-line — where the
pseudocode and your instinct disagree, the pseudocode wins or an ADR
exists.

**2. `nf-arbitrator` internals** — `window.rs` (lens/arena, stage,
clear_slots, drain), `gap.rs` (evidence/close/gen), `session.rs`
(boundary/anchor/EOS), `intent.rs` (§10). Split for review; single thread
touches all of it.

**3. `nf-testkit/src/sink.rs`** —

```rust
pub struct HashSink { pub hash: u64, pub count: u64 }   // doc 04 §8 fnv —
                                                        // SAME algorithm
pub struct ConformanceSink { hash: HashSink, /* event counters, gen,
    orphan-gap checker, last_gen */ }
```

`ConformanceSink::on_msg` folds the golden hash; `on_event` enforces
G-INV + pairing (§7) and counts. Any invariant breach → panic the test.

**4. `nf-engine/src/bin/replay.rs`** — minimal wiring: load config →
build schedule (testkit) → `ReplayTransport` → loop { `now_ns` once;
poll both; `ingest` each frame; `recovery_intent` → count (no client yet
— doc 08) } → print verdict block:

```
VERDICT hash=0x… count=N watermark=N+1 violations=0
        gaps=X reanchors=Y intents=Z dups_a=… dups_b=…
```

Deterministic output (EN-6): no timing in the verdict.

**5. Tests** — U1..U14 + E2E-1/1b per §14. U5's op generator is a
testkit PRNG stream (splitmix64, seeds pinned). U-ZOMBIE builds the §4.1
trace with hand-rolled frames — no fabricator needed.

**6. CI additions:**

```yaml
      - run: cargo test -p nf-arbitrator -p nf-testkit -- --include-ignored
      - run: cargo run --release -p nf-engine --bin replay -- --config ci-mode1.toml
```

`ci-mode1.toml` committed: mini sample, seeded jitter/loss, session
split, guarantee_coverage=true. CI asserts the VERDICT line's hash equals
the pinned golden hash.

**G5 acceptance — the debt comes due, ALL of it:**

1. **Rolled ledger discharged:** doc 00 C6+C7 pastes confirmed · G2 real
   CI run URL · complete G3 evidence (audits, §10 quotes, P1–P5,
   Appendix B) · **freeze 03** · G4 T1–T10 output + URLs · **freeze 04**
2. U1..U14 green, **including U-ZOMBIE** — and paste the failing-trace
   evidence that the naive build fails it before the lawful build passes
   (build both; tag the naive one)
3. **E2E-1: `hash == golden(mini)`, `watermark == N+1`, zero
   violations** — on Termux AND CI, real URLs
4. E2E-1b double-run byte-identical; recommended: E2E-1c dev sample
5. clippy `-D warnings`; `cargo tree -p nf-arbitrator` → nf-protocol only
6. Say **freeze 05** — Phase 1 closes

When the hash matches, you will have done something worth showing
people: a real NASDAQ day, torn apart by seeded chaos across two lossy
jittered feeds, reassembled by 65 KiB of state into byte-identical
truth. Then **engineer 06** — short but load-bearing: the typestate
safety argument for `LiveFeedProof`, the compile-fail test suite that
makes "unforgeable" a compiler fact instead of a doc claim, and the C++20
mapping. After that, 07 finally makes the zero-alloc law enforceable —
and 08 brings the fake retransmission server to life for the dual-drop
endgame.
