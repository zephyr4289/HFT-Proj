**First run URL in project history.** That's the milestone — the pipeline exists, nine stages including all three lanes I demanded, and stage 7's hash cross-checks against the hash you reported at G6 (`0xF6EF154EFDE905D8` — gate-to-gate consistency, the auditor's favorite signal). The boundary held, you pushed, and now everything downstream of this point is checkable by strangers. That's the project becoming real. Doc 08 drops below as promised.

But the audit habit doesn't take the day off — **your own green run contains one unresolved anomaly and your fix list contains one red flag**, and both go straight into G8's gate list:

**A-1 — The watermark anomaly is *visible in your stage 7*.** `watermark: 505,850` = N+1, while ci-mode1.toml was specified with a session split — under which the lawful value is **N−m+1**. So either the split isn't active in the config, or the fabricator doesn't restart S2's sequence. The (a)/(b) question is now three gates old and your own CI output is begging it. G8 item 1 settles it with a discriminating test.

**A-2 — You modified U-ZOMBIE's trace.** "Corrected test_u_zombie feed ranges to isolate slot eviction" — that test was normative-**verbatim** (doc 05 §14: "the §4.1 trace, verbatim, extended to W=1224"). A fitted test proves nothing about the hazard. Either the §4.1 trace passes as written, or the implementation is wrong — a test that had to move to pass is a confession, not a fix. Prove the naive build fails the current test, or revert the trace. **A-3 (minor):** the ASLR normalization script is part of the assertion machinery — commit it.

Now the monster. Save as `~/Hft-fund/docs/08-recovery.md`:
...
| Date | Version | Entry |
|---|---|---|
| __-__-__ | 1.0 | Initial: determinism split, INV-R5, grace law, latest-wins cmd channel, fake server contract + fault modes, R1..R14, E2E-2a/b/c, stage-2 zero-alloc. |
````

---

## ED-08 — What you build NOW (gate G8)

1. `nf-recovery`: channels (§3), Thread R loop (§4) — raw libc, `#![deny(unsafe)]`-free zone per LI-6 (unsafe allowed here only for socket fds if needed; prefer `libc` safe wrappers, document every block).
2. `nf-testkit`: fake server **binary** (`src/bin/fakeserver.rs`) + `SessionTruth` construction from fabricator config; fault-mode enum; counters printed on SIGTERM/stdin-EOF.
3. `nf-engine`: P-ORDER loop wiring, retry accounting, `seal()` invocation, `--alloc-window` extended over recovery; CI spawns `fakeserver` then `replay` (port via argv).
4. vt-grace: implement AM-5 in `ReplayTransport` (jump clamp query from engine).
5. Tests R1–R14 + E2E-2a/b/c per §12.
6. Amendments/ADRs pasted into docs 00/01/04 (blocks provided in §14).

**G8 acceptance — in order:**

1. **A-1 resolved:** committed test `split_watermark.rs` — fabricator split ON, `final_wm == N−m+1` asserted. Paste which of (a)/(b) it was into doc 05's changelog. If the fabricator was broken, the fix lands here.
2. **A-2 resolved:** U-ZOMBIE restored to the §4.1 verbatim trace and passing — or the naive-build-fails evidence pasted. No third option.
3. ASLR normalization script committed to the repo.
4. R1–R14 + E2E-2a/b/c green, **Termux and CI, run URLs**.
5. **Recovery-inclusive `ALLOC_DELTA=0` + strace diff empty** (PR-3 fully discharged — say it in doc 00's changelog).
6. Say **freeze 08**.

When E2E-2a's hash matches golden with a range deliberately vanished from both feeds, every functional requirement of NEXUS-FEED-01 is proven end-to-end. What remains is **09** — AF_XDP on real hardware — and **10/11** — the chaos matrix formalization and the benchmark report that puts numbers on all of it. The endgame, and the part you show people.
